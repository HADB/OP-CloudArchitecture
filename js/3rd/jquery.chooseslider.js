    $.UI = {};
    $.UI.slider = function(selection, options, callback) {
    	var _t = this;
      ...
      _t.lastValue = 0;
    	...
    	_t.showUnit = function() {
    		_t.unitDom.find('span').css('color', '#c8c1a8');
    	...
    	_t.widthToValue = function(width) {
    		...
    		_v = _t.block[i].min + Math.ceil(_w * ((unit.max - unit.min) / unit.unit)) * unit.unit; /* 调整值 */
        _t.lastValue = _t.currentValue;
    	...
    	_t.barDom.click(function(ev) {
    		...
    		width = Math.floor(ev.clientX + $(document).scrollLeft() - _t.distanceX);
    	...
    	_t.dragDom.mousedown(function(ev) {
    		...
    		_t.maxWidth;
    		_t.distanceX = _t.rangeDom.offset().left;
    		_t.diffX = ev.clientX + $(document).scrollLeft() - _t.dragDom.offset().left;
    		_t.ismousedown = true;
    	});
    	...
    	$(document).mousemove(function(ev) {
    		...
    		x = ev.clientX + $(document).scrollLeft() - _t.diffX - _t.distanceX;
